...
map.zoomControl.remove(); // Quitamos el zoom para liberar espacio en móvil
...
ubicaciones.forEach(sitio => {
    ...
    marcador.bindPopup(`<div class='popup-container'><img src='imagenes/${sitio.imagen}' class='popup-imagen'><div class='popup-texto'>${sitio.texto}</div></div>`);
    marcadoresCreados.push({ nombre: sitio.nombre.toLowerCase(), marker: marcador });
});

// Leyenda con e.stopPropagation para no bloquear el mapa
...
map.on('locationfound', e => L.marker(e.latlng).addTo(map).bindPopup("¡Estás aquí, mujer mas guapa del mundo mundial! ❤️").openPopup());